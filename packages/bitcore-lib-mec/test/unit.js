...
describe('Unit', function() {
  ...
  it('has property accesors "MEC", "mMEC", "uMEC", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'MEC');
    unit.MEC.should.equal(1.2);
    unit.mMEC.should.equal(1200);
    unit.uMEC.should.equal(1200000);
  ...
  it('a string amount is allowed', function() {
    ...
    unit = Unit.fromMEC('1.00001');
    unit.MEC.should.equal(1.00001);

    unit = Unit.fromMilis('1.00001');
    unit.mMEC.should.equal(1.00001);
    ...
    unit.MEC.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.MEC);
    Unit.MEC.should.equal('MEC');

    should.exist(Unit.mMEC);
    Unit.mMEC.should.equal('mMEC');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'MEC');
    unit.to(Unit.MEC).should.equal(unit.MEC);
    unit.to(Unit.mMEC).should.equal(unit.mMEC);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'MEC');
    unit.toMEC().should.equal(unit.MEC);
    unit.toMilis().should.equal(unit.mMEC);
    unit.toMillis().should.equal(unit.mMEC);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'MEC'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'MEC').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'MEC').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
