...
export class SearchProvider {
  ...
  private inputTypes: InputType[] = [
    ...
    // Standard BTX address
    {
      regexes: [/^(bitcore:)?([23][a-km-zA-HJ-NP-Z1-9]{25,34})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
        { chain: 'BTX', network: 'mainnet' }
      ],
    },
    // bech32 BTX Address
    {
      regexes: [/^(bitcore:)?(btx1[ac-hj-np-zAC-HJ-NP-Z02-9]{11,71})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
        { chain: 'BTX', network: 'mainnet' }
    ...
    // BTC / BCH / BTX / DOGE / ETH block height
    {
      regexes: [/^[0-9]{1,9}$/],
      type: 'block',
      chainNetworks: [
