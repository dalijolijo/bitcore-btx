...
export class SearchProvider {
  ...
  private inputTypes: InputType[] = [
    ...
    // Standard MEC address
    {
      regexes: [/^(megacoin:)?([M][a-km-zA-HJ-NP-Z1-9]{25,34})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
        { chain: 'MEC', network: 'mainnet' }
      ],
    },
    // bech32 MEC Address
    {
      regexes: [/^(megacoin:)?(mec1[ac-hj-np-zAC-HJ-NP-Z02-9]{11,71})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
        { chain: 'MEC', network: 'mainnet' }
  ...
  private extractAddress(address: string): string {
    const extractedAddress = address
      .replace(/^(bitcoincash:|bchtest:|bitcoin:|megacoin:)/i, '')
      .replace(/\?.*/, '');
