...
import { BitcoreLib, BitcoreLibMec, BitcoreLibCash } from 'crypto-wallet-core';

var Bitcore = BitcoreLib;
var BCHAddress = BitcoreLibCash.Address;
var MECAddress = BitcoreLibMec.Address;
...
export class Verifier {
  ...
  static checkPaypro(txp, payproOpts) {
    ...
    if (txp.coin == 'mec' && (new MECAddress(toAddress).toString()) != (new MECAddress(payproOpts.instructions[0].toAddress).toString()))
      return false;
