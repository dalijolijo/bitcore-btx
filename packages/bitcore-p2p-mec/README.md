...
**The peer-to-peer networking protocol for MEC.**

`bitcore-p2p-mec` adds [Bitcore protocol](https://en.bitcoin.it/wiki/Protocol_documentation) support for Bitcore.
...
```sh
npm install bitcore-p2p-mec
```

In order to connect to the Bitcore MEC network, you'll need to know the IP address of at least one node of the network, or use [Pool](./docs/pool.md) to discover peers using a DNS seed.

```javascript
var Peer = require('bitcore-p2p-mec').Peer;
